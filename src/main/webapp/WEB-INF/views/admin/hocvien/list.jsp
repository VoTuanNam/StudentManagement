								...
								<div class="table-btn-controls">
									<div class="pull-right tableTools-container">
										<div class="dt-buttons btn-overlap btn-group">
											<c:url var="createHocVienURL" value="/quan-tri/bai-viet/chinh-sua" />
											<a flag="info"
												class="dt-button buttons-colvis btn btn-white btn-primary btn-bold"
												data-toggle="tooltip" title='Thêm học viên' href='${createHocVienURL}'> <span>
											...
											<tbody>
												<c:forEach var="item" items="${model.listResult}">
													<tr>
														...
														<td>${item.faculty}</td>
														<td>${item.gender}</td>
														<td  hidden>${item.classUITCode}</td>
														<td  hidden>${item.facultyCode}</td>
														<td>
															<c:url var="updateHocVienURL" value="/quan-tri/bai-viet/chinh-sua">
																	<c:param name="id" value="${item.id}"/>															
															</c:url>
															
															<a class="btn btn-sm btn-primary btn-edit"
																data-toggle="tooltip" title="Cập nhật học viên" href='${updateHocVienURL}'><i
		...
		function deleteHocVien(data) {
	        $.ajax({
	            ...
	            success: function (result) {
	                window.location.href = "${hocvienURL}?page=1&limit=10&message=delete_success";
	            },
